...
t=zeros(1,n);
nonzero_A=27*m;
t(1:n) = (6*n-1)*((1:n)-1)+19*m-4+2*nonzero_A;
%C_memory=zeros(1,n);
%C_memory(1:n-1)=(2*(1:n-1)+4)*m;
%C_memory(n)=4*m;
%C=CCR*C_memory;
%R=C;
C_memory=4*m*ones(1,n);
C=CCR*C_memory;

R_memory=zeros(1,n);
R_memory(1:n-1)=(2*(1:n-1)+4)*m;
R_memory(n)=4*m;
R=CCR*R_memory;
...
for i_total=1:5
    clearvars -except CCR P_total i_total t T C R D n ratio k_star_total p_opt_pattern_total time barplot_data_total%p_save
    P=P_total(i_total)
    repeat_times=100;
...
%     MS_YD_C_min=zeros(1,repeat_times);
    ...
    for i_rep=1:repeat_times  
        ...
        [~,i_min_ckpt]=min(R); % only for GCR, incremental checkpointing leads to constant, search the minimal R cost (actually is the last task) 
...
%csvwrite('data_GCR_n10_CCR2_temp.csv',ratio,1,0)
%eval(['csvwrite(''data_GCR_n' num2str(n) '_CCR' num2str(CCR) '_temp.csv'',ratio,1,0)'])
...
eval(['save data_GCR_n' num2str(n) '_CCR' num2str(CCR) '_new.mat n k_star_total p_opt_pattern_total ratio time ratio_median barplot_data_total'])
