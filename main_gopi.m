...
D=5;
ratio=zeros(5*100*3,1);
k_star_total=zeros(1,5);
p_opt_pattern_total=zeros(1,5);

for i_total=1:5
    clearvars -except n P_total i_total t T C R D ratio k_star_total p_opt_pattern_total %p_save
    P=P_total(i_total)
    repeat_times=100;
    ...
    ratio(1+(i_total-1)*repeat_times*3:i_total*repeat_times*3,1) = [ratio1'; ratio2'; ratio3'];
    
    k_star_total(i_total)= k_star;
    p_opt_pattern_total(i_total)= p_opt_pattern;
end
csvwrite('data_gopi.csv',ratio,1,0)
save data_gopi.mat n k_star_total p_opt_pattern_total ratio
tt=toc
