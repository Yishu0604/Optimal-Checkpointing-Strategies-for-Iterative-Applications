function [t] = simulation_optimal_pattern_perturb(N,F,W,C,C_pattern,R_pattern,D,k_seg,n,w_length,i_w,R)
t = 0; %time 
...
while k_seg <= N*n  
    if  k_seg <= N*n-w_length(i)+1
        ...
        if  t+sum(W(k_seg:k_seg+w_length(i)-1))+C_pattern(i) <= F(m) %success of iteration i
            %t = t+W_pattern(i)+C_pattern(i);
            t = t+sum(W(k_seg:k_seg+w_length(i)-1))+C_pattern(i);
